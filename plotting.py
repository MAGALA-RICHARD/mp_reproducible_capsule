...
def relplot(data, show=False, filename=None, annotation_text='', add_scatter=True, legend_loc='upper left', bbox_to_anchor=(0.98, 0.98), **kwargs):
    ...
    if annotation_text:
        bg = g.ax.get_facecolor()
        g.ax.text(
            0.01, 0.99, annotation_text,  # top-left in axes coords
            transform=g.ax.transAxes,
            ha="left", va="top",  # anchor text from its top-left
            bbox=dict(facecolor=bg, alpha=0.75, edgecolor="none", pad=6),
            fontsize=14,
            family="DejaVu Sans Mono"
        )
    # rest of your code unchanged
    fileName = filename or RESULTS / f"{kwargs.get('x')}_{kwargs.get('y')}-{kwargs.get('hue')}-{kwargs.get('kind')}-{kwargs.get('height')}{PlotConstants.FILE_EXTENSION}"
    ...
    if show:
        open_file(fileName)
    doc_figure =RESULTS / f"{kwargs.get('x')}_{kwargs.get('y')}-{kwargs.get('hue')}-{kwargs.get('kind')}-{kwargs.get('height')}.png"
    plt.savefig(doc_figure, bbox_inches="tight")
    add_image(doc_figure, y=kwargs.get("y"), x=kwargs.get('x'), hue=kwargs.get('hue'), save=True)
    plt.close()
    return g
...
def cat_plot(data, title="", show=False, legend_loc='upper left', bbox_to_anchor=(0.10, 0.98), **kwargs):
    ...
    # rest of your code unchanged
    base_script_name =Path(__file__).stem
    fileName = RESULTS / f"{base_script_name}-{kwargs.get('x')}_{kwargs.get('y')}-{kwargs.get('hue')}-{kwargs.get('kind')}-{kwargs.get('height')}{PlotConstants.FILE_EXTENSION}"
...
def mva_plot(data, **kwargs):
    ...
    g = plot.plot_mva(table=da, response='SOC_0_15cm_Mg', time_col='year', hue='timing', palette='tab10',
    ...
    fileName = RESULTS / f"{kwargs.get("time_col")}_{kwargs.get("y")}-{kwargs.get('hue')}-{kwargs.get('response')}-{kwargs.get('height')}{PlotConstants.FILE_EXTENSION}"
...
if __name__ == '__main__':
    ...
    da = merge_tables(str(RESULTS / 'single_model.db'), tables=['carbon', 'yield'])
    ...
    mva_plot(data=da, response='SOC_0_15cm_Mg', grouping=("Nitrogen", 'Residue', 'timing'), hue='timing',
             errorbar=None,
             col="Residue", col_wrap=2)
    mva_plot(data=da, response='grainwt', grouping=("Nitrogen", 'Residue', 'timing'), hue='timing',
             errorbar=None,
    ...
    # )
    dat = da
    cat_plot(data=dat, y='top_mineralized_N', x='Nitrogen', hue='timing', kind='bar', title='')
    cat_plot(data=dat, y='Incorporated_Biomass_Mg', x='Nitrogen', hue='timing', kind='bar', title='')
    cat_plot(data=dat, y='Incorporated_Biomass_Mg', x='Residue', hue='timing', kind='bar', title='',
             bbox_to_anchor=(0.10, 1))

    cat_plot(data=dat, y='Surf_Org_Carbon_Mg', x='Nitrogen', hue='timing', kind='box', title='', n_boot=500,
             legend='full', legend_out=True)
    dt = dat.groupby(['Nitrogen', 'Residue', 'timing', ], observed=True)['Surf_Org_Carbon_Mg'].mean().reset_index()
    cat_plot(data=dt, y='Surf_Org_Carbon_Mg', x='Residue', hue='timing', kind='bar', title='',
             legend='full', legend_out=True, legend_loc='upper right', bbox_to_anchor=(0.98, 0.98))

    cat_plot(data=dat, y='ce', x='Residue', hue='timing', kind='bar', title='',
             legend='full', legend_out=True, legend_loc='upper right', bbox_to_anchor=(0.98, 0.98))

    cat_plot(data=dat, y='corn_yield_Mg', x='Nitrogen', hue='timing', kind='box', title='', )
    cat_plot(data=dat, y='corn_yield_Mg', x='Residue', hue='timing', kind='bar', title='', )
    cat_plot(data=dat, y='SOC_0_15cm_Mg', x='Nitrogen', hue='timing', kind='bar', title='')
    cat_plot(data=dat, y='cnr', x='Nitrogen', hue='timing', kind='box', title='', estimator='mean',
    ...
    cat_plot(data=dat, y='cnr', x='Residue', hue='timing', kind='bar', title='', estimator='mean',
             legend_loc='upper right', bbox_to_anchor=(0.98, 0.98), )
    cat_plot(data=dat, y='top_carbon_mineralization', x='Residue', hue='timing', kind='bar', title='',
    ...
    cat_plot(data=c_change, y='ΔSOC_0_15CM', x='Nitrogen', hue='timing', kind='box', title='',
             estimator='mean', legend_loc='upper right', bbox_to_anchor=(0.98, 0.98), )
    ...
    res = mean_diff_between_windows(da, grouping=['Nitrogen', 'timing', ], sample=False, sample_size=10,
                                    sample_window=20,
                                    col='corn_yield_Mg', last_window=last_window, first_window=first_window, min_obs=5)

    change_reb = mean_diff_between_windows(da, grouping=['Nitrogen', 'timing', ], sample=True, sample_window=20,
                                           sample_size=5,
                                           col='Residue_Biomass_Mg', last_window=last_window, first_window=first_window,
                                           min_obs=5)
    mic_soc15 = mean_diff_between_windows(da, grouping=['Nitrogen', 'timing', 'Residue'], sample=True, sample_window=20,
    ...
    c_change = compute_last_minus_first_change(data=da, col='SOC_0_15CM', grouping=('Nitrogen', 'Residue', 'timing'))
    cat_plot(data=c_change, y='ΔSOC_0_15CM', x='timing', hue='Nitrogen', kind='bar', title='',
             estimator='mean', legend_loc='upper right', bbox_to_anchor=(0.2, 0.98), )

    c_change = compute_last_minus_first_change(data=da, col='SOC_0_15CM', grouping=('Nitrogen', 'Residue', 'timing'))
    cat_plot(data=c_change, y='ΔSOC_0_15CM', x='timing', hue='Residue', kind='bar', title='',
             estimator='mean', legend_loc='upper right', bbox_to_anchor=(0.2, 0.98), )

    c_change = compute_last_minus_first_change(data=da, col='SOC_0_15CM', grouping=('Nitrogen', 'Residue', 'timing'))
    cat_plot(data=c_change, y='ΔSOC_0_15CM', x='Nitrogen', hue='timing', kind='bar', title='',
             estimator='mean', legend_loc='upper right', bbox_to_anchor=(0.2, 0.98), )
    cat_plot(data=res, show=True, y='%Δcorn_yield_Mg', x='Nitrogen', hue='timing', kind='bar', title='',
             estimator='mean', legend_loc='upper right', bbox_to_anchor=(0.2, 0.98), )

    cat_plot(data=change_reb, show=False, y='%ΔResidue_Biomass_Mg', x='Nitrogen', hue='timing', kind='bar', title='',
             estimator='mean', legend_loc='upper right', bbox_to_anchor=(0.2, 0.98), )
    doc_path = RESULTS / "seaborn_plots_report.docx"
