...
Created on Sunday Oct 18 2025:

this is the old script, the new one is defined in qp_model_soc_balance.py
"""
import math
import os
...
def fit_quadratic_plateau(x, y, weights=None):
    ...
    ls = least_squares(resid, p0, bounds=(lb, ub), jac='2-point', method='dogbox', verbose=2)  # robust & fast
...
def contour_plot(**kwargs):
    ...
    fig_name = kwargs.get("fig_name", 'contour_plot.png')
    show = kwargs.get("show", True)
    color_bar_label = kwargs.get("color_bar_label", 'z values')
    ...
    contour = plt.tricontourf(x, y, z, cmap=c_map, levels=level_s, vmin=z.min(), vmax=z.max())
    colorbar = plt.colorbar(contour, label=color_bar_label, orientation=orientation)
...
def contour_plot_nitrogen_res_interaction(data, filter_lower_n_rates=False, threshold=100, x='Residue', y='Nitrogen', z='SOC_0_15CM', nlevels=200):
    fi_name = f"{filter_lower_n_rates}.png"
    dfg = data.copy()

    if filter_lower_n_rates:
        dfg = dfg[dfg['Nitrogen'].astype(float) > threshold].copy()
    xx, yy, zz =dfg[x].astype(float), dfg[y].astype(float), dfg[z].astype(float)
    print(type(xx), type(yy), type(zz))
    contour_plot(x=xx, y=yy, z=zz, n_levels=nlevels, xlabel=LABELS.get(x, x), show=False,
                 color_bar_label= LABELS[z], cmap='viridis_r', fig_name=fi_name,
                 ylabel=LABELS.get(y, y))

def fit_qp_robust_median(x, y, min_left=4, min_right=4):
    """
    Robust quadratic-plateau:
      For x <= x0: y = P + c (x - x0)^2   with c <= 0
      For x >  x0: y = P                   (plateau)

    Strategy:
      - For each candidate x0 with data on both sides:
          P = median(y_right)
          c = sum((y_left - P)*d2) / sum(d2^2), where d2=(x_left - x0)^2
          clamp c<=0
          compute SSE over all points
      - Pick x0 with min SSE
    Returns dict with params (a,b,c,x_break,plateau) and basic metrics.
    """
    ...
    if n < (min_left + min_right + 1):
        raise ValueError("Not enough points on both sides of breakpoint.")

    best = None
    # candidate breakpoints only at interior x positions
    for j in range(min_left, n - min_right):
        x0 = x[j]

        # left and right masks (strict right so plateau uses true right-side)
        L = x <= x0
        R = x > x0
        if L.sum() < min_left or R.sum() < min_right:
            continue

        # robust plateau from right side
        P = float(np.median(y[R]))

        # closed-form c on left
        d2 = (x[L] - x0) ** 2
        num = np.sum((y[L] - P) * d2)
        den = np.sum(d2 ** 2)
        c = (num / den) if den > 0 else 0.0
        if c > 0:  # enforce concavity
            c = 0.0
        ...
        if (best is None) or (sse < best[0]):
            best = (sse, P, c, x0, yhat)

    if best is None:
        raise RuntimeError("No valid breakpoint with enough data on both sides.")

    sse, P, c, x0, yhat = best
    # map to (a,b,c) for the left quadratic branch
    a = P + c * x0 ** 2
    b = -2.0 * c * x0
    ...
    r2 = 1.0 - sse / sst if sst > 0 else np.nan
    rmse = float(np.sqrt(sse / max(n - k, 1)))

    return {
        "params": {"a": a, "b": b, "c": c, "x_break": x0, "plateau": P},
        "metrics": {"n": n, "SSE": sse, "R2": r2, "RMSE": rmse},
        "predict": lambda x_new: P + c * np.where(np.asarray(x_new, float) <= x0,
                                                  (np.asarray(x_new, float) - x0) ** 2, 0.0)
    }
...
if __name__ == '__main__':
    ...
    for residue in sorted(soc_balance.Residue.unique()):
        ...
        res = fit_qp_robust_median(x=df['N'], y=df[RESPONSE])
    ...
    # print(corr_table)
    ##########################################################################################################
    # trying to plot it in the same plane
    ...
    #############################################################
    y_lab = "Soil organic carbon balance (Mg ha⁻¹) after 101 years"
    g.fig.text(
    ...
    open_file(fn)
    plt.savefig(f"{RESULTS}/{MODULE_NAME}_fitted_SOC_figure.svg", bbox_inches="tight")  # Windows vector
    # or
