...
from settings import N_RATES, RESIDUE_RATES, RESULTS
from apsimNGpy.settings import logger
from mgt_reporter import make_report, make_mgt_text
import matplotlib.pyplot as plt
from utils import create_experiment, mva, plot_mva
...
model2 = create_experiment(base_file='base_split.apsimx', lonlat=lonlatTest)
...
try:
    ...
    raise ie
model2.results.to_csv(scratch_DIR / f'{TILLAGE_DEPTH}_split_nitrogen.csv', index=False)
...
plot_trend_mva(single_model, 'SOC_0_15CM', table='carbon', ylabel=soc_mg_yl)
...
if __name__ == '__main__':
    cat_plot(single_model, 'carbon', 'DUL(2)')
    ...
    test_mva(model2, table='carbon', response='min_ratio', expression="min_ratio =  SOC_0_15CM", time_col='year',
             grouping=("Residue", 'Nitrogen'), estimator='mean',
             errorbar=None, col='Residue', col_wrap=2, hue="Nitrogen")

    # single_model.preview_simulation()
    data = single_model.get_simulated_output('carbon')
    ...
    # single_model.preview_simulation()
    test_mva(model2, table='yield', response='grainwt', time_col='year',
             grouping=("Residue"), estimator='mean',
    ...
    single_model.cat_plot(table=data, y='cnr', x='N', kind='bar', hue='R')
    plt.title("Single experiment")
    plt.savefig(f'single{TILLAGE_DEPTH}.png')
    os.startfile(f'single{TILLAGE_DEPTH}.png')
    plt.close()

    data_split['N'] = pd.Categorical(data_split['Nitrogen'], ordered=True)
    data_split['R'] = pd.Categorical(data_split['Residue'], ordered=True)
    model2.cat_plot(table=data_split, y='cnr', x='N', kind='box', hue='R', )
    plt.title('split experiment')
    plt.savefig(f'split{TILLAGE_DEPTH}.png')
    os.startfile(f'split{TILLAGE_DEPTH}.png')
    plt.close()
    ...
    sns.catplot(data=ch, y='dSOC_0_15CM', x='Residue', hue='Nitrogen', kind='bar', col='timing')
    fname = f"soc_change{TILLAGE_DEPTH}.png"
    plt.savefig(fname)
    os.startfile(fname)
    # view(single_model.get_simulated_output('carbon_change'))
    yd = single_model.get_simulated_output('yield')
    ...
    test_mva(model2, table=yd, expression=None, response='RBiomass', time_col='year',
             grouping=("Residue", 'Nitrogen'), estimator='mean',
             errorbar=None, col='Residue', col_wrap=2, hue="Nitrogen")
    yd['Nitrogen'] = pd.Categorical(yd['Nitrogen'], ordered=True)
    yd.sort_values(by='Residue', ascending=True, inplace=True)
    cat_plot(model2, table=yd, expression=None, y='biomass', x='Residue',
    ...
    cat_plot(model2, table=carbon, expression=None, y='SOC_0_15CM', x='Residue',
